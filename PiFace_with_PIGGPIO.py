...
# USING PIGPIO DRIVER
pi1 = pigpio.pi()
#pi1.set_servo_pulsewidth(4, 1000)
...
def P0():	# Process 0 controlles servo0
	...
	_Servo0CP = 999		# by making the current position and desired position unequal,-
	_Servo0DP = 1000		# 	we can be sure we know where the servo really is. (or will be soon)
	while True:
		time.sleep(speed)
		...
		if _Servo0CP < _Servo0DP:					# if Servo0CP less than Servo0DP
			...
			pi1.set_servo_pulsewidth(4, _Servo0CP)
			# ServoBlaster.write('Server' + str(_Servo0CP) + '\n')	#
			# ServoBlaster.flush()					#
...
def P1():	# Process 1 controlles servo 1 using same logic as above
	...
	while True:
		...
		if _Servo1CP < _Servo1DP:
			_Servo1CP += 10
			Servo1CP.put(_Servo1CP)
			pi1.set_servo_pulsewidth(17, _Servo1CP)
